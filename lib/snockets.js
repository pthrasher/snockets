(function() {
  var BEFORE_DOT, CoffeeScript, DIRECTIVE, DepGraph, EXPLICIT_PATH, HEADER, HoldingQueue, Snockets, compilers, fs, jsExts, parseDirectives, path, stripExt, timeEq, uglify, _;
  ...
  module.exports = Snockets = (function() {
    ...
    Snockets.prototype.scan = function(filePath, flags, callback) {
      ...
      this.readFile(filePath, flags, __bind(function(err) {
        if (err) {
          return callback(err);
        }
        ...
        return this.updateDirectives(filePath, flags, __bind(function(err) {
          ...
          if (callback) {
            return callback(null, this.depGraph);
          } else {
    ...
    Snockets.prototype.getCompiledChain = function(filePath, flags, callback) {
      ...
      return (_ref = flags.async) != null ? _ref : flags.async = this.options.async;
    };
    ...
    Snockets.prototype.findMatchingFile = function(filename, flags, callback) {
      var tryFiles;
      tryFiles = __bind(function(filePaths, required) {
        var err, filePath, _i, _len;
        for (_i = 0, _len = filePaths.length; _i < _len; _i++) {
          filePath = filePaths[_i];
          if (stripExt(this.absPath(filePath)) === filename) {
      ...
      if (tryFiles(this.cache, false)) {
        return;
      }
      return this.readdir(path.dirname(filename), flags, __bind(function(err, files) {
        if (err) {
          return callback(err);
        }
        return tryFiles(files, true);
      }, this));
    };
    ...
    Snockets.prototype.updateDirectives = function() {
      var callback, command, depList, directive, excludes, filePath, flags, q, relPath, relPaths, require, requireTree, words, _i, _j, _k, _l, _len, _len2, _len3, _ref;
      ...
      q = new HoldingQueue({
        task: __bind(function(depPath, next) {
          ...
          return this.readFile(depPath, flags, __bind(function(err) {
            if (err) {
              return callback(err);
            }
      ...
      require = __bind(function(relPath) {
        ...
        if (relName.match(EXPLICIT_PATH)) {
          ...
          return this.findMatchingFile(this.absPath(depName), flags, function(err, depPath) {
            if (err) {
              return callback(err);
            }
            return q.perform(relName, depPath);
  ...
  module.exports.compilers = compilers = {
    coffee: {
      match: /\.js$/,
      compileSync: function(filePath, source) {
  ...
  stripExt = function(filePath) {
    return BEFORE_DOT.exec(filePath)[1];
  };
  jsExts = function() {
    var ext;
    return ((function() {
